...
public class Event extends javax.swing.JPanel {
   ...
   private ArrayList<Model.Event> list;
    public Event(Student student) {
        initComponents();
        list = new EventDAO().getListEvent(); 
    ...
    private void initComponents() {
        ...
        jPanel1.setLayout(new java.awt.CardLayout(30, 30));
        ...
        table.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "Mã sự kiện", "Mã sinh viên", "Ngày tổ chức", "Mô tả"
            }
        ) {
    ...
    private void btnHienThiMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_btnHienThiMouseClicked
        EventDAO eventDAO = new EventDAO(); // Changed from DutyScheduleDAO to EventDAO
        List<Model.Event> events = eventDAO.getAll(); // Changed from getAll to use Event
        DefaultTableModel model = (DefaultTableModel) table.getModel();
        model.setRowCount(0);
        for (Model.Event event : events) { // Changed from DutySchedule to Event
            Object[] rowData = {
                event.getEventId(), // Changed from getScheduleId to use Event
                event.getStudentId(), // Changed from getStudentId to use Event
                event.getEventDate(), // Changed from getScheduleDate to use Event
                event.getDescription() // Changed from getDescription to use Event
        };
...
public void exportExcelFile() {
    try {
        ...
        XSSFSheet sheet = excelFile.createSheet("DANH SÁCH SỰ KIỆN"); // Changed from "DANH SÁCH LỊCH TRỰC" to "DANH SÁCH SỰ KIỆN"

        // Header row
        String[] headers = {"STT", "Mã sự kiện", "Mã sinh viên", "Ngày sự kiện", "Mô tả"}; // Changed from "STT", "Mã lịch trực", "Ngày trực" to "STT", "Mã sự kiện", "Ngày sự kiện"
        Cell cell;
        ...
        // Data rows
        int rowNum = 1;
        for (Model.Event event : list) { // Changed from DutySchedule to Event
            row = sheet.createRow(rowNum++);
            row.createCell(0, CellType.NUMERIC).setCellValue(rowNum - 1); // STT
            row.createCell(1, CellType.NUMERIC).setCellValue(event.getEventId()); // Mã sự kiện
            row.createCell(2, CellType.STRING).setCellValue(event.getStudentId()); // Mã sinh viên
            row.createCell(3, CellType.STRING).setCellValue(event.getEventDate().toString()); // Ngày sự kiện
